...
import com.example.flappy.single.AppwriteClientSingleton
import com.example.flappy.utils.Comment
import com.example.flappy.utils.SingleLiveEvent
import com.example.flappy.utils.Tweet
import com.example.flappy.utils.generateRandomString
...
class AppwriteRepository(val context: Context) {
    ...
    private val _commentCounts = MutableLiveData<Map<String, Int>>()
    val commentCounts: LiveData<Map<String, Int>> = _commentCounts
...
//            val curUser = curUserId /*?: throw Exception("User not found")*/
    ...
    suspend fun addUser(user: com.example.flappy.utils.User) {
        val databases = Databases(client)
        val documentId = ID.unique()
        try {
...
//            val curUser = curUserId /*?: throw Exception("User not found")*/
    ...
    suspend fun getSelectedTweetComments(tweetId: String): DocumentList<Map<String, Any>> {
        return try {
            ...
            databases.listDocuments(
                databaseId = Constant.DATABASE_ID,
                collectionId = Constant.COLLECTION_COMMENTS_ID,
                queries = listOf(Query.equal(Constant.DATA_TWEET_ID_DEL, tweetId))
            )
        } catch (e: Exception) {
            Log.e("Search Comments", e.message.toString())
            DocumentList(
                total = 0,
                documents = emptyList(),
            )
    ...
    suspend fun fetchCommentCountsForTweets(tweetIds: List<String>) {
        ...
        try {
            // Single batched query to fetch all comments for given tweetIds
            val result = databases.listDocuments(
                databaseId = Constant.DATABASE_ID,
                collectionId = Constant.COLLECTION_COMMENTS_ID,
                queries = listOf(
                    Query.or(tweetIds.map { Query.equal("tweetId", it) })
                )
            )

            // Convert documents to Comment objects
            allComments.addAll(result.documents.map {
                Comment(
                    tweetId = it.data["tweetId"] as String,
                    ...
                    replyingTo = it.data["replyingTo"] as? String
                )
            })

            val resultMap = allComments
                .filter { it.tweetId != null }
                .groupingBy { it.tweetId!! }
                .eachCount()
            _commentCounts.postValue(resultMap)

        } catch (e: Exception) {
            Log.e("CommentCount", "Failed to fetch batched comment counts: ${e.message}")
        }
    }
    ...
    suspend fun postComment(comment: Comment) {
        ...
        try {
            databases.createDocument(
                ...
                documentId = documentId,
                data = mapOf(
                    "tweetId" to comment.tweetId,
                    "userId" to comment.userId,
                    "username" to comment.username,
                    "text" to comment.text,
                    "timestamp" to comment.timestamp,
                    "replyingTo" to comment.replyingTo
                )
            )
            Toast.makeText(context, "Commented", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
            Toast.makeText(context, "Error creating comment", Toast.LENGTH_SHORT).show()
    ...
    suspend fun getComments(tweetId: String): List<Comment> {
        ...
        val result = databases.listDocuments(
            databaseId = Constant.DATABASE_ID,
            collectionId = Constant.COLLECTION_COMMENTS_ID,
            queries = listOf(Query.equal("tweetId", tweetId)),
        )
